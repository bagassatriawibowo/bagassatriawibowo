...
for x in range(len(det)):
    if negara == det[x]['name']:
        index_negara = x
mid_col.header('Informasi Negara yang Anda Pilih')
...
mid_col.write(f'Produksi Terbesar Terjadi pada Tahun: {dat_terbesar}')
mid_col.write(f'Produksi Terkecil Terjadi pada Tahun: {dat_terkecil}')
sum_data = zasdsad['Produksi minyak mentah'].sum()
mid_col.write(f'Produksi Kumulatif Keseluruhan Tahun dari Negara Ini yakni Sebesar: {sum_data:.2f}')
