...
min_total_produksi = np.asarray(total_prod).min()
min_total_produksi_idx = np.asarray(total_prod).argmin()
min_produksi=np.asarray(sum_produksi).min()
min_produksi_idx = np.asarray(sum_produksi).argmin()
right_col.markdown(f"Negara dengan total produksi paling sedikit adalah : {negara_negara2[min_total_produksi_idx]} ({min_total_produksi}) dan pada tahun {tahun} produksi paling sedikitnya yakni sebesar:{negara_negara2[min_produksi_idx]} ({min_produksi})")
